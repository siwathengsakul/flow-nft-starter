...
function App() {
  ...
  useEffect(() => {
    //Listens to changes in user && updates the user
    ...
    try {
      const transactionId = await fcl.mutate({
        ...
        args: (arg, t) => [
          ...
          arg("CatMoji No." + _id.toString(), types.String), // Name
          arg("Cat Emojis on the bc", types.String), // Description
      ...
      console.log(err)
      alert("Error minting NFT, please check the console!")
    }
  }
  ...
  const fetchNFTs = async () => {
    setImages([]) // empty the image array
    let IDs = []

    // fetch the IDs with our script (no fee or signers!)
    try {
      IDs = await fcl.query({
        cadence: `${getIDs}`,
        args: (arg, t) => [
    ...
    try {
      for (let i = 0; i < IDs.length; i++) {
        ...
        // if the source is an IPFS, remove "ipfs://"
    ...
    if (images.length < _imageSrc.length) {
      setImages((Array.from({length: _imageSrc.length}, (_, i) => i).map((number, index) =>
        <img style={{margin: "10px", height: "150px"}} src={_imageSrc[index]} key={number} alt={"NFT No." + number} />
        )))
    }
  }
